...
namespace LogicalTier.TicketPricing
{
    ...
    public class LastMinutePricing : ITicketPricing 
    {

        // The price's discount increases as more days pass since the Occurrence's 'CreateDate'
        ...
        public int CalculatePrice(Event anEvent, Occurrence occurrence)
        {
            ...
            int daysFromCreationToToday = (int)(DateTime.Now - occurrence.CreateDate.ToDateTime(TimeOnly.MinValue)).TotalDays;

            double discount; // the discount in percentage
            if (daysFromCreationToOccurrence == 0) 
                discount = 0; 
            else 
                discount = ((double)daysFromCreationToToday / daysFromCreationToOccurrence) * LogicalTier.Constants.MAX_PRICE_DISCOUNT;               
